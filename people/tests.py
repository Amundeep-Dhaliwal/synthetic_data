...
class PersonAPIClassTests(TestCase):
    ...
    async def test_get(self):
        request_data = ujson.dumps({"number": 1})
    ...
    def test_get_number_of_people(self):
        ...
        with patch.object(
            self.view,
            "raise_response",
            side_effect=RaisedResponse(self.content_tested_dict, 412),
        ) as raiser:
            self.assert_response(self.view.get_number_of_people, 412, number_omitted)
    ...
    def test_validate_input_fields(self):
        ...
        incorrect_field = ["nme"]
        suggestion_tuple = self.view.validate_inputted_fields(
            self.view, incorrect_field
        )
        self.assertEqual(suggestion_tuple[2], "name")

        incomprehensible_field = ["green"]
        no_suggestion_tuple = self.view.validate_inputted_fields(
            self.view, incomprehensible_field
        )
    ...
    def test_process_get_request(self):
        ...
        with (
            ...
            patch.object(
                self.view,
                "handle_fields",
                return_value=deepcopy(self.input_and_output_queries),
            ) as fields,
            patch.object(
                self.view, "handle_age_restrictions", return_value=np.arange(10)
            ) as array,
            patch.object(
                self.view,
                "raise_response",
                side_effect=[
                    RaisedResponse(self.content_tested_dict, 201),
                    RaisedResponse(self.content_tested_dict, 500),
                ],
            ) as raiser,
            patch(
                "people.views.PersonSerializer",
                side_effect=[valid_serializer, invalid_serializer],
            ) as serializer,
        ):
            self.assert_response(
    ...
    def test_handle_fields(self):
        ...
        with (
            patch.object(
                self.view, "raise_response", side_effect=RaisedResponse({}, 412)
            ) as raiser,
            patch.object(
                self.view,
                "validate_inputted_fields",
                side_effect=[(False, 0, 0), (True, 0, 0), (True, 0, 0)],
            ) as validation,
        ):
            incomplete_request_body = {"fields": ["name"]}
            self.assert_response(self.view.handle_fields, 412, incomplete_request_body)
            ...
            self.assert_response(self.view.handle_fields, 412, age_not_specified)
    ...
    def test_handle_age_restrictions(self):
        ...
        with patch.object(
            self.view, "raise_response", side_effect=RaisedResponse({}, 416)
        ) as raiser:
            lower_age_limit_without_age_field = {"age_lower_limit": 10}
            self.assert_response(
                self.view.handle_age_restrictions,
                416,
                lower_age_limit_without_age_field,
            )
...
class PersonDataClassTests(TestCase):
    ...
    def test_person_generation(self):
        ...
        no_age_specified_student = generate_persons(
            1, student_fields, output_fields, student_age_range
        )
